...
for A in $(ls | awk '{print $NF}'); do
    ...
    if wp redis info --allow-root; then
        ...
        status="Failed"
        echo "$A: $status"
        curl -X POST -H "Content-Type: application/json" -d '{"ip":"'"$(curl -s ifconfig.me)"'","application":"'"$A"'","status":"'"$status"'"}' https://phpstack-1246017-4482242.cloudwaysapps.com/disabled
    fi

    # Log the result to the API
#    curl -X POST -H "Content-Type: application/json" -d '{"ip":"'"$(curl -s ifconfig.me)"'","application":"'"$A"'","status":"'"$status"'"}' https://phpstack-1246017-4482242.cloudwaysapps.com/applications
